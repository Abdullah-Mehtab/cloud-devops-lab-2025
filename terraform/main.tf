...
locals {
  ...
  # VARIABLE: app_ports
  # Map of application names to port numbers that should be accessible from the bastion
  app_ports = {
    nginx       = 80
    jenkins     = 8080
    sonarqube   = 9001
...
module "security" {
  ...
  app_ports    = local.app_ports      # Map of application ports for security group rules
}
